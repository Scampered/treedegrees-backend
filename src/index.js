    ...
    const { rows } = await pool.query(
      `SELECT p.* FROM admin_popups p
       WHERE (p.target = 'all' OR p.target = $1)
         ...
         AND p.id NOT IN (SELECT popup_id FROM popup_dismissals WHERE user_id = $1)
       ORDER BY p.created_at DESC`,
      [payload.id]
    );
    res.json(rows.map(p => ({ id: p.id, header: p.header, subheader: p.subheader, buttonText: p.button_text })));
  } catch { res.json([]); }
});
...
app.post('/api/popups/:id/dismiss', apiLimiter, async (req, res) => {
  try {
    ...
    const payload = verifyToken(header.slice(7));
    await pool.query(
      `INSERT INTO popup_dismissals (popup_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
...
// ── Admin "am I admin?" — before adminRoutes ──────────────────────────────────
app.get('/api/admin/me', apiLimiter, async (req, res) => {
  try {
    ...
    const payload = verifyToken(header.slice(7));
    const { rows } = await pool.query(
      'SELECT is_admin FROM users WHERE id = $1 AND deleted_at IS NULL', [payload.id]
    );
