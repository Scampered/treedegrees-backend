// src/utils/jobPollers.js — timed delivery for courier, farmer 1h refund, steward rebill
...
async function pollCourierDeliveries() {
  ...
  try {
    ...
    for (const r of rows) {
      await client.query(
        `UPDATE courier_requests SET status='delivered', delivered_at=NOW() WHERE id=$1`, [r.id]
      );
      ...
      notify(r.requester_id, 'job_hired', '📬 Delivery complete',
        `Your courier delivery is done. ${r.recipient_label ? 'To: ' + r.recipient_label : ''}`, '/jobs').catch(() => {});
      sendPush(r.courier_id, '✅ Delivery complete',
        `Delivered: ${r.from_country} → ${r.to_country}`, '/jobs').catch(() => {});
    }
    if (rows.length > 0) console.log(`[courierPoller] delivered ${rows.length} request(s)`);
...
async function pollFarmerRefunds() {
  ...
  try {
    ...
    for (const r of rows) {
      await client.query(`UPDATE users SET seeds=seeds+$1 WHERE id=$2`, [r.seeds_deposited, r.depositor_id]);
      await client.query(
        `UPDATE farmer_plots SET status='empty', seeds_deposited=0, depositor_id=NULL, planted_at=NULL WHERE id=$1`,
        [r.id]
      );
      ...
      sendPush(r.depositor_id, '🌾 Seeds refunded',
        `${r.seeds_deposited} seeds returned — farmer didn't plant within 1 hour.`, '/jobs').catch(() => {});
    }
    if (rows.length > 0) console.log(`[farmerPoller] refunded ${rows.length} deposit(s)`);
  } catch(e) { console.error('[farmerPoller]', e.message); }
  finally { client.release(); }
}

// ── Steward: expire contracts + notify, also auto-renew if client has seeds ───
async function pollStewardContracts() {
  ...
  try {
    ...
    for (const sc of expired) {
      // Auto-renew if client has enough seeds
      if (sc.client_seeds >= sc.fee_seeds) {
        const newExpiry = new Date(Date.now() + sc.retainer_days * 24 * 3600000);
        await client.query(`UPDATE users SET seeds=seeds-$1 WHERE id=$2`, [sc.fee_seeds, sc.client_id]);
        await client.query(`UPDATE users SET seeds=seeds+$1 WHERE id=$2`, [sc.fee_seeds, sc.steward_id]);
        await client.query(
          `UPDATE steward_clients SET last_paid_at=NOW(), expires_at=$1 WHERE id=$2`,
          [newExpiry, sc.id]
        );
        notify(sc.client_id, 'job_hired', '🔔 Steward renewed',
          `Your ${sc.retainer_days}-day steward contract auto-renewed for 🌱${sc.fee_seeds}.`, '/jobs').catch(() => {});
        notify(sc.steward_id, 'job_hired', '🔔 Contract renewed',
          `A client renewed their steward contract.`, '/jobs').catch(() => {});
      } else {
        // Can't afford — expire and refund nothing (already paid)
        await client.query(`UPDATE steward_clients SET status='expired' WHERE id=$1`, [sc.id]);
        notify(sc.client_id, 'job_hired', '🔔 Steward contract ended',
          `Your steward contract expired — not enough seeds to renew.`, '/jobs').catch(() => {});
        notify(sc.steward_id, 'job_hired', '🔔 Client contract ended',
          `A steward contract expired (client out of seeds).`, '/jobs').catch(() => {});
        sendPush(sc.client_id, '🔔 Steward contract ended',
          `Your ${sc.retainer_days}-day contract has ended.`, '/jobs').catch(() => {});
      }
    }
    if (expired.length > 0) console.log(`[stewardPoller] processed ${expired.length} contract(s)`);
  } catch(e) { console.error('[stewardPoller]', e.message); }
  finally { client.release(); }
}

// ── Broker: auto-close sessions whose time has expired ───────────────────────
  ...
  try {
    ...
    for (const session of sessions) {
      ...
      for (const alloc of allocs) {
        ...
        await client.query(`UPDATE broker_allocations SET settled=true, settled_at=NOW(), return_amount=$1 WHERE id=$2`, [clientGet, alloc.id]);
      }
      ...
      await client.query(`UPDATE users SET seeds=seeds+$1 WHERE id=$2`, [totalBrokerCut, session.broker_id]);
      await client.query(`UPDATE broker_sessions SET status='settled', escrow_seeds=0 WHERE id=$1`, [session.id]);

      notify(session.client_id, 'job_hired', '🌱 Broker session closed',
        `Session ended. You received 🌱${totalClientGet}.`, '/jobs').catch(() => {});
      notify(session.broker_id, 'job_hired', '🌱 Session auto-closed',
        `Time expired. Client got 🌱${totalClientGet}, you earned 🌱${totalBrokerCut}.`, '/jobs').catch(() => {});
    }
    if (sessions.length > 0) console.log(`[brokerPoller] closed ${sessions.length} session(s)`);
...
export function startJobPollers() {
  // Courier: every 5 minutes
  pollCourierDeliveries();
  setInterval(pollCourierDeliveries, 5 * 60 * 1000);

  // Farmer: every 10 minutes
  pollFarmerRefunds();
  setInterval(pollFarmerRefunds, 10 * 60 * 1000);

  // Steward: every 15 minutes
  pollStewardContracts();
  setInterval(pollStewardContracts, 15 * 60 * 1000);

  // Broker: every 5 minutes
  pollBrokerSessions();
  setInterval(pollBrokerSessions, 5 * 60 * 1000);

  console.log('✅ Job pollers started (courier/farmer/steward/broker)');
}
